        ...
        <>
            <div className="flex items-center justify-between">
                <PageHeader
                    breadcrumb={
                        <Link to="/datasets"
                              className="mb-2 inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-brand-600">
            ...
            {!error && (
                <div className="space-y-6">
                    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                        ...
                        <StatCard icon={FiColumns} label="Columns" value={formatNumber(analytics.data?.columnCount)}
                                  tone="violet"/>
                        <StatCard icon={FiLayers} label="Missing values"
                                  value={formatNumber(analytics.data?.missingValueCount)} tone="amber"/>
                        <StatCard icon={FiAlertOctagon} label="Invalid values"
                                  value={formatNumber(analytics.data?.invalidValueCount)} tone="rose"/>
                    </div>
                    ...
                    <div className="grid gap-4 lg:grid-cols-3">
                        ...
                        <Card title="Summary" subtitle="GET /analytics/datasets/:id/summary">
                            {loading ? <Loader rows={6}/> :
                                <SummaryPanel summary={summary.data} analytics={analytics.data}/>}
                        </Card>
                    ...
                    <div className="grid gap-4 lg:grid-cols-3">
                        <Card title="Type distribution">
                            {loading ? <Loader rows={5}/> :
                                <TypeDistributionChart distribution={summary.data?.columnTypeDistribution || {}}/>}
                        </Card>
                    ...
                    <Card title="Columns" subtitle="Open a column for its full profile" bodyClassName="pt-0">
                        {loading ? <div className="pt-5"><Loader rows={6}/></div> :
                            <ColumnTable datasetId={datasetId} statistics={stats}/>}
                    </Card>
                </div>
            )}
        </>
